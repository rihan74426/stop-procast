...
const DELETED_KEY = `stopprocast_deleted_ids_v${SCHEMA_VERSION}`;
...
export function loadDeletedIdsLocal() {
  if (typeof window === "undefined") return [];
  try {
...
export function saveDeletedIdsLocal(ids = []) {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(
      DELETED_KEY,
      JSON.stringify({ version: SCHEMA_VERSION, ids })
    );
  } catch (e) {
    console.warn("Momentum: localStorage write failed (deleted ids)", e);
  }
}

export function saveDeletedIdLocal(id) {
  if (typeof window === "undefined") return;
  try {
    const ids = loadDeletedIdsLocal();
    if (!ids.includes(id)) {
      ids.push(id);
...
export function clearDeletedIdsLocal() {
  if (typeof window === "undefined") return;
  try {
...
export async function loadProjectsRemote() {
  try {
    // Pass session ID so anonymous projects are included in the response
    const headers = {};
    const sessionId = getSessionId();
    if (sessionId) headers["X-Session-Id"] = sessionId;
