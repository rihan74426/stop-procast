...
// ─── GENERATE_MODELS ─────────────────────────────────────────────────
// Curated: prefer multilingual instruction-following and robust models.
// Order: best free → reliable free/cheap → last resort
...
// ];
// ─── UTILITY_MODELS ───────────────────────────────────────────────────
// Clarify / reengage — prefer very fast, small models
const UTILITY_MODELS = [
...
function isValidResponse(text, isGenerateTask = false) {
  ...
  // Detect obvious garble
  if (/0\.0\.0\.0\.0\.0/.test(t) || /asdfasdf|lorem ipsum/i.test(t))
    return false;

  if (isGenerateTask) {
    // Prefer strict JSON: try parse first
    try {
      const parsed = JSON.parse(t);
      // Basic schema checks: projectTitle and phases
      if (
        parsed &&
      ...
      // If parsed but doesn't match, reject
      return false;
    } catch {
      // If not pure JSON, fall back to weak heuristics:
