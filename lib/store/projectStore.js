...
// AFTER — attach to the store state itself:
// Remove the module-level variable.
// In the store definition, add to initial state:
export const useProjectStore = create(
  immer((set, get) => ({
    ...
    // Call this from DataProvider on mount, not at import time:
    loadLocal() {
      if (get()._localLoaded) return;
    ...
    _hydratedForUserId: null, // ADD THIS

    async hydrateFromServer() {
      // REPLACE the hydratedSession guard with:
      const hydrationKey =
      ...
      if (get((s) => s._hydratedForUserId === hydrationKey)) return;

      set((s) => {
        s._hydratedForUserId = hydrationKey;
      });
...
export function resetHydratedSession() {
  useProjectStore.setState({ _hydratedForUserId: null });
}
