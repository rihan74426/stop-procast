...
export const useToastStore = create((set, get) => ({
  ...
  show(
    message,
    { type = "info", duration = 4000, action, dedup = false, icon = null } = {}
  ) {
    ...
    const id = ++_id;
    set((s) => ({
      toasts: [...s.toasts, { id, message, type, action, customIcon: icon }],
    }));
...
export const toast = {
  info: (msg, opts) =>
    useToastStore
      .getState()
      .show(msg, { type: "info", duration: 4000, ...opts }),
  success: (msg, opts) =>
    useToastStore
      .getState()
      .show(msg, { type: "success", duration: 3000, ...opts }),
  error: (msg, opts) =>
    useToastStore
      .getState()
      .show(msg, { type: "error", duration: 5000, ...opts }),
  warn: (msg, opts) =>
    useToastStore
      .getState()
      .show(msg, { type: "warn", duration: 4000, ...opts }),
  loading: (msg, opts) =>
    useToastStore
      .getState()
